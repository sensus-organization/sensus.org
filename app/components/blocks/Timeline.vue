...
import type { HighlightedText, TimelineEvent } from "~/types/strapi";
...
const { data: events } = await useStrapiFind<TimelineEvent>("timeline-events", {
    sort: "order",
...
function getColorClasses(color?: string | null) {
    const colors: Record<string, { bg: string; border: string; text: string; dot: string }> = {
        red: { bg: "bg-sensus-red/10", border: "border-sensus-red", text: "text-sensus-red", dot: "bg-sensus-red" },
        blue: { bg: "bg-sensus-blue/10", border: "border-sensus-blue", text: "text-sensus-blue", dot: "bg-sensus-blue" },
        teal: { bg: "bg-sensus-teal/10", border: "border-sensus-teal", text: "text-sensus-teal", dot: "bg-sensus-teal" },
        orange: { bg: "bg-sensus-orange/10", border: "border-sensus-orange", text: "text-sensus-orange", dot: "bg-sensus-orange" },
        green: { bg: "bg-sensus-green/10", border: "border-sensus-green", text: "text-sensus-green", dot: "bg-sensus-green" },
    };
    return colors[color && Object.hasOwn(colors, color) ? color : "red"];
}
</script>
    ...
    <section class="py-12 md:py-16 relative overflow-hidden">
        ...
        <div class="relative z-10 max-w-6xl mx-auto px-6">
            <div class="text-center mb-10">
                <p v-if="label" class="text-sensus-red font-semibold text-sm uppercase tracking-widest mb-3">
                    {{ label }}
                </p>
                <h2 v-if="title" class="heading-section text-3xl md:text-4xl lg:text-5xl text-sensus-gray-900 mb-4">
                    <BaseHighlightedTitle :content="title" />
                </h2>
                <p v-if="description" class="text-sensus-gray-600 text-lg max-w-2xl mx-auto">
                    {{ description }}
                </p>
            </div>
            ...
            <div class="relative">
                ...
                <div class="space-y-6 md:space-y-8">
                    <div v-for="(event, index) in events || []" :key="`${event.year}-${index}`" class="timeline-item relative">
                        <div class="hidden md:grid items-start" style="grid-template-columns: 80px 24px 1fr; gap: 16px">
                            ...
                            <div class="flex-1">
                                <component
                                    ...
                                    :to="strapiLink(event.link) || undefined"
                                    class="group card-elevated p-6 md:p-8 border-l-4 relative"
                                    :class="[
                                        ...
                                        event.link ? 'block cursor-pointer' : '',
                                    ]"
                                >
                                    <div
                                        v-if="event.current"
                                    ...
                                    <h3
                                        class="text-xl md:text-2xl font-bold text-sensus-gray-900 mb-3 transition-colors"
                                        :class="event.link ? 'group-hover:text-sensus-red' : ''"
                                    >
                                        {{ event.title }}
                                    </h3>
                        ...
                        <div class="md:hidden">
                            <component
                                :is="event.link ? NuxtLink : 'div'"
                                :to="strapiLink(event.link) || undefined"
                                class="group bg-white rounded-2xl border border-sensus-gray-200 shadow-sm transition-all duration-300 hover:shadow-xl hover:-translate-y-1 p-6 border-l-4 relative"
                                :class="[
                                    getColorClasses(event.color).border,
                                    event.current ? 'ring-2 ring-sensus-red/20' : '',
                                    event.milestone ? 'bg-gradient-to-br from-white to-sensus-gray-50' : '',
                            ...
                            >
                                ...
                                <h3
                                    class="text-lg font-bold text-sensus-gray-900 mb-3 transition-colors"
                                    :class="event.link ? 'group-hover:text-sensus-red' : ''"
                                >
                                    {{ event.title }}
                                </h3>
